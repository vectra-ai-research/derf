...
## Handle the 'USER' parameter so the detection can be run as different users
  try:
    if 'USER' in data:
      ...
      include_hdrs = list('host', 'content-type', 'date', 'x-amz-*', 'Content-MD5')
      auth = AWS4Auth(accessKeyId,accessKeySecret, data['REGION'], data['SERVICE'], include_hdrs=include_hdrs)  
    else:
